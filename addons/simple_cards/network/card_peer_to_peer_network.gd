## Peer-to-peer multiplayer coordinator for Simple Cards.
##
## Add this as a scene node, commonly named CardNetwork, in scenes where each
## peer is allowed to author and broadcast its own card actions. This mode is
## useful for trusted games, local/LAN games, or games where the transport layer
## already provides validation. It is not cheat-resistant.
## @experimental: Multiplayer support may change before it is considered stable.
class_name CardPeerToPeerNetwork extends CardNetworkManager
...
func should_route_move(_card: Card, _target: CardContainer) -> bool:
	return _should_route_local_command()
...
func should_broadcast_local_action() -> bool:
	return enabled and not is_applying_remote_state() and _broadcast_depth == 0


func _should_route_local_command() -> bool:
	if not enabled:
		return false
	if is_applying_remote_state():
		return false
	return _has_multiplayer_peer()
...
func ensure_card_id(card: Card) -> StringName:
	...
	if card.is_inside_tree() and card.owner:
		...
		return scene_id
	var id := StringName("sc_p2p_card_%s_%d_%d" % [
...
func request_move(card: Card, target: CardContainer, config: Card.MoveConfig = null) -> void:
	...
	await _apply_and_broadcast_local_command(command)
...
func request_move_cards(cards: Array[Card], source: CardContainer, target: CardContainer, config: Card.MoveConfig = null) -> int:
	...
	return int(await _apply_and_broadcast_local_command(command))
...
func request_shuffle(pile: CardPile) -> void:
	if not pile:
		return
	if not _sender_can_command_container(_get_local_peer_id(), pile):
		command_rejected.emit(_build_command(COMMAND_SHUFFLE_PILE, pile, pile, [], null), "source_not_allowed")
		return
	begin_suppressed_routing()
	pile._shuffle_local()
	end_suppressed_routing()
	var command := _build_command(COMMAND_SET_CONTAINER_ORDER, pile, pile, pile.cards, null)
	command.type = COMMAND_SHUFFLE_PILE
	_broadcast_local_state(command, -1.0)
...
func request_slot_swap(slot: CardSlot, other_slot: CardSlot) -> bool:
	...
	return bool(await _apply_and_broadcast_local_command(command))
...
func request_set_card_data(card: Card) -> void:
	...
	command.metadata["card_state"] = card.get_network_state(_get_local_peer_id())
	await _apply_and_broadcast_local_command(command)
...
func queue_revision_broadcast() -> void:
	bump_revision_and_broadcast.call_deferred()


func broadcast_state(animation_duration: float = 0.0) -> void:
	if not enabled or not _has_multiplayer_peer():
		return
	_cleanup_registries()
...
func _request_card_command(command: Dictionary) -> void:
	...
	if sender_id == 0:
		sender_id = _get_local_peer_id()
	await _apply_remote_command(command, sender_id)
...
func _apply_and_broadcast_local_command(command: Dictionary) -> Variant:
	var validation_reason := _validate_command(command, _get_local_peer_id())
	if not validation_reason.is_empty():
		command_rejected.emit(command, validation_reason)
		return false
	...
	end_suppressed_routing()
	_broadcast_depth = maxi(0, _broadcast_depth - 1)

	var config := _get_move_config_from_command(command)
	_broadcast_local_state(command, config.duration)
	return value


func _apply_remote_command(command: Dictionary, sender_id: int) -> void:
	var validation_reason := _validate_command(command, sender_id)
	if not validation_reason.is_empty():
		command_rejected.emit(command, validation_reason)
		if multiplayer.get_peers().has(sender_id):
			_command_rejected_remote.rpc_id(sender_id, command, validation_reason)
		return

	state_revision = maxi(state_revision, int(command.get("revision", 0)))
	begin_suppressed_routing()
	await _apply_command_local(command)
	end_suppressed_routing()
	...
	if config.duration < 0.0:
		for container in _containers_by_id.values():
			if is_instance_valid(container):
				(container as CardContainer).arrange()


func _broadcast_local_state(command: Dictionary, animation_duration: float = 0.0) -> void:
	if not enabled:
		return
	state_revision += 1
	command.revision = state_revision
...
func _apply_command_local(command: Dictionary) -> Variant:
	...
	match type:
		...
		COMMAND_DEAL_TO:
			...
			return 0
		COMMAND_REORDER_HAND, COMMAND_SET_CONTAINER_ORDER, COMMAND_SHUFFLE_PILE:
			if target:
...
func _validate_command(command: Dictionary, sender_id: int) -> String:
	...
	match type:
		...
		COMMAND_MOVE_CARD, COMMAND_MOVE_CARDS:
			...
			return ""
		COMMAND_FLIP_CARD, COMMAND_SET_CARD_DATA:
			if cards.is_empty():
				return "missing_cards"
			for card in cards:
				if not card:
...
func _apply_snapshot_payload(snapshot: Dictionary) -> void:
	...
	state_revision = maxi(state_revision, int(snapshot.get("revision", 0)))
