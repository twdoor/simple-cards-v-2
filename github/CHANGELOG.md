...
- **Per-Container Focus Toggle** — `CardContainer` exposes a new `cards_focusable: bool = true` export. When `false`, all cards in the container get `focus_behavior_recursive = FOCUS_BEHAVIOR_DISABLED` regardless of shape, and controller/keyboard input passes through to surrounding Controls (e.g. buttons placed on top of a discard pile). Replaces the earlier shape-based `ContainerShape.cards_focusable()` which forced all stack-shaped containers — including solitaire foundations — to lose focus.
...
- **Layout Resolution with Fallbacks** — `CardGlobal.create_layout()` now accepts a second `fallback_id` argument and resolves through requested → fallback → `def_front_layout` → built-in default. New helpers `has_layout()`, `get_layout_path()`, and `resolve_layout_id()` expose the resolution logic. `Card` records the resolved ID on a new `current_layout_name` property and emits `layout_changed` with the resolved ID instead of the requested one, so callers can tell when a fallback kicked in.
- **StringName Layout IDs** — `Card.layout_name`, `front_layout_name`, and `back_layout_name` are now `StringName` instead of `String`. Matches the rest of the layout API and avoids per-frame string allocation when layouts switch.
- **Rotation-Aware Container Bounds** — `ContainerShape.compute_layout()` and `CardContainer._get_minimum_size()` both compute the bounding box using rotated card corners. Arc and other rotated shapes are now correctly centered inside their container instead of being offset by the rotated extent of the end cards.
- **Container Idle Stopped on Cleanup** — `CardContainer._exit_tree()` and the `NOTIFICATION_PREDELETE` handler now call `_stop_idle()`, and `CardHand`/`CardSlot` do the same in their own `_exit_tree`. Looping idle tweens no longer leak when a container is freed mid-animation.
- **BobCardAnimation Cleanup** — `BobCardAnimation` listens for `CardLayout.tree_exiting` and kills/erases the per-layout tween, fixing a leak when cards are freed while the bob loop is active.
- **CardHand Focus Signal Tracking** — `CardHand` now stores its `bind()`-ed focus callables in dictionaries so `_disconnect_card_signals()` can actually disconnect them. Previously the bound callables were rebuilt on disconnect and the original connections were left dangling.
- **CardPile Auto-Registers Children** — `CardPile._container_ready()` now picks up `Card` children placed under the pile in the editor and registers them, matching `CardSlot`'s existing behavior.
- **CardSlot Drop Rejection Reasons** — `CardSlot._handle_drop()` separately emits `card_rejected` with `"slot_locked"`, `"failed_conditions"`, or `"slot_full"` instead of a single generic rejection. `can_accept_card()` no longer special-cases full slots with `allow_swap`; swap handling lives entirely in `_handle_drop`.
- **CardSlot Swap Preserves Source Index** — `_swap_cards()` records the incoming card's index in its source container and re-registers the displaced card at the same index, so swaps from a hand keep the existing card order. The source container also receives matching `card_removed`/`card_added` signals.
- **Layout Cache Hardening** — `LayoutCache` normalizes layout IDs and tags (trimmed, deduped, sorted), validates IDs with `is_valid_identifier()`, sanitizes generated `LayoutID` constant names for non-identifier characters, errors on collisions, and writes the cache file with stable sorted ordering. New `_upsert_layout_metadata()` inserts `metadata/is_layout`, `metadata/layout_id`, and `metadata/tags` if they are missing instead of silently no-oping.
- **Layout Panel Path Validation** — The create dialog now rejects paths outside `res://` and paths that aren't `.tscn`. The preview thumbnail renders the layout in-place (positioned offscreen with mouse filter disabled) instead of detaching the `SubViewport`, fixing edit-mode crashes when SubViewports were reparented.
- **Shape Edge-Case Guards** — `ArcShape` clamps `arc_radius` away from zero, `GridShape` clamps `num_of_rows`/`num_of_cols` to at least 1, and `LineShape` clamps computed spacing to non-negative. Prevents divide-by-zero and overlap glitches when these are set to zero in the inspector.
- **Safer Autoload Registration** — `SimpleCards._enter_tree()` skips `add_autoload_singleton()` if `autoload/CardGlobal` is already registered, and `_exit_tree()` only removes it if this plugin registered it. Avoids clobbering an existing autoload of the same name.

### Breaking Changes

- `Card.layout_name`, `Card.front_layout_name`, and `Card.back_layout_name` are now `StringName` instead of `String`. Code reading these as `String` should still work via implicit conversion; code writing with `String` literals continues to work but prefer `&"id"` for clarity.
- `CardGlobal.create_layout(layout_id)` → `CardGlobal.create_layout(layout_id, fallback_id = &"")`. Adding the optional second argument is non-breaking, but the fallback chain now prefers `def_front_layout` over the hardcoded default — set `def_front_layout` explicitly if you relied on the old fallback.
...
- **Idle Container Animations** — `CardContainer` now has an `idle_animation` export. Assign a looping `CardAnimationResource` to play an ambient animation on all cards while they sit in the container. Idle is started automatically when cards are added and stopped per-card when dragging begins.
- **BobCardAnimation** — New built-in `CardAnimationResource` that makes cards bob up and down with configurable `amplitude`, `frequency`, and `phase_variance` for a wave effect across the hand. Animates `CardLayout` offsets so it never conflicts with the layout system or dragging.
